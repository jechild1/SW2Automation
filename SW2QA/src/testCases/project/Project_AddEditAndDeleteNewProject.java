...
/**
 * Test to add a new project to a client and then to edit that project. Finally,
 * it will delete that same project. <br>
 * This test will exercise the Add Project modal. It will:<br>
 * <li>Ensure that the project is not currently created. Anything that says
 * "Delete Me" will be removed.
 ...
 * functionality
 * <li>Ensure that the newly created project can be edited.
 * <li>Ensure that the values on the edited project commit.
 ...
 *
 */
public class Project_AddEditAndDeleteNewProject extends BaseTestScriptConfig {
	...
	public void addNewProject() {
		 ...
		 * Perform edits
		 */
		// Page is left on the inspections page.
		// Lets go back to the projects page.
		projectInspectionsMainPage = new ProjectInspectionsPageFactory();
		projectInspectionsMainPage.clickProject();
		...
		// Assign the new data values for the next record in the datasheet.
		assignDataValues(projectsFile);

		// Set the new fields (the edit project is already displayed)
		setCreateNewProjectFields(projectPage);
	...
	private void assignDataValues(ExcelDataConfig projectsFile) {
		...
		// There are two rows in this datasheet. It starts as 0 and then each time adds
		// one. This way, we get to row two for the edit.
		rowIndex = rowIndex += 1;
	...
	private void deleteExistingProjects() {
		...
		List<WebElement> projects = new ArrayList<WebElement>();
		...
		do {
			...
			// In the Projects.xlsx datasheet, on the ProjectsForDeletion worksheet, all of
			// the project names start with "Delete Me", e.g. "Delete Me - Project B". The
			// following list will use an xpath that can find all of them.
			projects = driver
					.findElements(By.xpath("//h3[contains(text(),'Delete Me')]//parent::div[@class='project-main']"));
